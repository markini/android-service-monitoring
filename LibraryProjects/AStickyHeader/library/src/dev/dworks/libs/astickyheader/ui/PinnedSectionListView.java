...
public class PinnedSectionListView extends ListView {
	...
	private OnScrollListener mOnScrollListener = new OnScrollListener() {
		...
		public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
			...
			if (listAdapter == null || visibleItemCount == 0) {
				return; // nothing to do
			}

			//PinnedSectionListAdapter adapter = (PinnedSectionListAdapter)((ContextualUndoAdapter)listAdapter).getDecoratedBaseAdapter();
	...
	private final DataSetObserver mDataSetObserver = new DataSetObserver() {
		...
		public void onChanged() {
			PinnedSectionListAdapter adapter = (PinnedSectionListAdapter) ((ContextualUndoAdapter)getAdapter()).getDecoratedBaseAdapter();
	...
	public void onRestoreInstanceState(Parcelable state) {
		...
		post(new Runnable() {
			...
			public void run() {
				...
				if (adapter == null) {
					return; // nothing to restore as adapter is still null
				}
	...
	public void setAdapter(ListAdapter adapter) {
		...
		ListAdapter oldAdapter = null;
		// assert adapter in debug mode
		if (BuildConfig.DEBUG && adapter != null) {
			if (adapter instanceof ContextualUndoAdapter) {
				oldAdapter = adapter;
				adapter = ((ContextualUndoAdapter) adapter).getDecoratedBaseAdapter();
			}
